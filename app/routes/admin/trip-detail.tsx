...
const TripDetail = ({loaderData}: Route.ComponentProps) => {
    ...
    const pillItems = [
            ...
            {text: groupType, bg:'!bg-success-50 !text-success-500'},
            {text: budget, bg:'!bg-primary-50 !text-primary-500'},
            {text: interests, bg:'!bg-navy-50 !text-navy-500'}
    ]

    const visitTimeAndInfo = [
        {title: 'Best time to visit', items: bestTimeToVisit},
        {title: 'Weather',items: weatherInfo}
    ]
  return (
    <main className="travel-detail wrapper">
        <Header title="Trip Details" description="View and edit AI-generated travel plans"></Header>
        <section className="container wrapper-md">
            ...
            <section className="gallery">
                {imageUrls.map((url:string,i:number) => (
                    <img src={url}
                    key={i}
                    className={cn('w-full rounded-2xl object-cover',i===0? 'md:col-span-2 md:row-span-2 h-[330px]': 'md:row-span-1 h-[150px]')}/>
                ))}
            </section>
